#!/usr/bin/env node
/**
 * Evaluate the Linear (quantile regression) bundle on the test split.
 * Mirrors eval-gbdt.mjs so compare.mjs can treat all methods uniformly.
 *
 * Usage:
 *   node scripts/eval-linear.mjs [--json] [--models=~/.costea/models/linear]
 */
...
async function main() {
  ...
  const modelsDir = getArg("models", path.join(os.homedir(), ".costea", "models", "linear"));

  if (!json) console.error(`Loading Linear bundle from ${modelsDir}…`);
  const bundle = await loadLinearBundle(modelsDir);
  if (!bundle) {
    console.error(`No Linear bundle at ${modelsDir}. Run: python3 training/train_linear.py`);
    process.exit(2);
  }
  ...
  if (!skipCalib) {
    ...
    for (const t of split.val) {
      ...
      const p = predictLinearBundle(bundle, x);
  ...
  for (const t of split.test) {
    const x = encodeTask(t, bundle.manifest);
    const raw = predictLinearBundle(bundle, x);
    for (const tgt of TARGETS) {
      let p = raw[tgt];
  ...
  const out = {
    method: skipCalib ? "linear_quantile" : "linear_quantile_calibrated",
